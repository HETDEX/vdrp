...
def extract_fluxlim_spectra(args):
    ...
    for r_off in range(0, 70):
        ...
        for d_off in range(0, 70):
            ...
            starobs, _ = phot.get_star_spectrum_data(ra, dec, args, False, dithall)
    ...
    with open('list', 'w') as f:
        for i in range(0, n_wave):
            ...
            np.savetxt('w%d.j4' % wl, allspec[i, w[0], :], fmt="%.5f %.5f %.3f %.3f" )
            f.write('%s' % 'w%d.j4\n' % wl)
    ...
    outname = args.results_dir + '/' +  args.nightshot + '_' + args.fname + '.fits'
